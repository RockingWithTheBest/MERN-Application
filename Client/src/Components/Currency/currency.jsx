...
const columns = (DeleteById,UpdateById , GetById) => [
    ...
    },
    {
        ...
        cell: row => (
            <>
                <Button variant="outlined" startIcon={<DeleteIcon /> } onClick={()=>DeleteById(row._id)}>
                    Delete
                </Button>
            </>    
        )
...
const Currency = ()=>{
    ...
    const Currencies = async ()=>{
        const response = await axios.get("http://localhost:1000/currency",{
            params:{
    ...
    const DeleteById = async (id)=>{
        if(confirm("Are you sure?")){
            const url = `http://localhost:1000/currency/${id}`;
            try{
   ...
   const handleUpdate = async ()=>{
    ...
    if(editId){
        const url = `http://localhost:1000/currency/${editId}`;
        const data = {
            currency_name: currency_name,
            selling_rate: selling_rate,
            buying_rate: buying_rate,
            ...
            <div className="filter">
                <form action="" onSubmit={handledAddCurrency} className="form-submit">
                        ...
                        <div>
                            ...
                            <input className="fullname" type="number" value={selling_rate} onChange={(e)=>setSellingRate(e.target.value)} />
                        </div>
                        <div>
                            <label htmlFor="">Buying Rate : </label>
                            <input className="fullname" type="number" value={buying_rate} onChange={(e)=>setBuyingRate(e.target.value)} />
                        </div>
                        <div>
            ...
            {error && <p>Error: {error}</p>}
             ...
             <Popup open = {open} onClose ={()=> setOpen(false)} model nested>
                    <div className="update">
                            <h2>Update Info...</h2>
                            <label htmlFor="">Currency Name</label><input type="text" onChange={(e)=>setCurrencyName(e.target.value)} />
