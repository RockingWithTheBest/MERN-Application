...
const Route = ()=>{
    ...
    const Clients = async ()=>{
        const response = await axios.get("http://localhost:1000/client",{
            params:{
    ...
    const postData = async (e)=>{
        e.preventDefault();
        const url = "http://localhost:1000/client";
        const data = {
            full_name: fullname,
    ...
    const DeleteById = async (id)=>{
        if(confirm("Are you sure?")){
            const url = `http://localhost:1000/client/${id}`;
            try{
   ...
   const handleUpdate = async ()=>{
    ...
    if(editId){
        const url = `http://localhost:1000/client/${editId}`;
        const data = {
            full_name: fullname,
            passport_number: passportNumber
        };
